 ...
 */
public class Copy_2_of_Scanner implements IMatchContext {
    /** File contents. */
    ...
    public Copy_2_of_Scanner(byte[] input) {
        this.input = input;
    ...
    private MemoEntry recall(final Rule rule, final int beginPos) {
        ...
        if (h != null) {
            ...
            if (m == null && !h.rule.equals(rule) && !h.involvedSet.contains(rule)) {
                m = new MemoEntry(FAIL, beginPos);
            }

            // Allow involved rules to be evaluated, but only once during
            // seed-growing
            // iteration.
            else if (h.evalSet.contains(rule)) {
                h.evalSet.remove(rule);
                final IMatchResult answer = rule.eval(this);
                // / assert answer != FAIL || this.pos == pos :
                // "failed but advanced: answer=" + answer + " pos=" + pos + "->" +
                // this.pos;
    ...
    public <T> T applyRule(final String key, final IAction<T> body) throws MatchFailureException {
        ...
        IMatchResult r = applyRule(rule, K0, beginPos, null);
        System.out.println("trying " + key + " at " + beginPos + " (" + (beginPos < input.length ? (char) input[beginPos] : '$') + ")");
        while (r instanceof Continuation) {
            try {
                T answer = body.apply(this);
                r = applyRule(rule, (Continuation) r, beginPos, new Success(answer));
            }
            catch (MatchFailureException e) {
                r = applyRule(rule, (Continuation) r, beginPos, FAIL);
            }
        }
     ...
     */
  public  static class Continuation implements IMatchResult {
        int k;
        MemoEntry m;
        LR lr;
        Head h;

        public Continuation(int k) {
            this.k = k;
        }
        public Continuation(int k, MemoEntry m, LR lr, Head h) {
            this.k = k;
            this.m = m;
            this.lr = lr;
            this.h = h;
    ...
    // int beginPos = context$.pos;
    // Continuation k = new Continuation(0, 0);
    // IMatchResult answer = null;
    // while (true) {
    //     r = applyRule("A", k, answer);
    //     if (r instanceof Continuation) {
    //         k = (Continuation) r;
    //         answer = p.A();
    //     }
    //     else
    ...
    static final Continuation K0 = new Continuation(0);
    static final Continuation K1 = new Continuation(1);
    static final Continuation K2 = new Continuation(2);
    static final Continuation K3 = new Continuation(3);
    
    public IMatchResult applyRule(final Rule rule, Continuation k, int beginPos, IMatchResult answer) {
        switch (k.k) {
        case 0: {
            assert beginPos == this.pos;
            MemoEntry m = memo[beginPos] != null ? memo[beginPos].get(rule) : null;
            final Head h = heads[beginPos];
            ...
            if (m == null) {
                ...
                return new Continuation(2, m, lr, null);
            }
        ...
        case 1: {
            MemoEntry m = new MemoEntry(answer, beginPos);
        ...
        case 2: {
            ...
            if (lr.head != null) {
                ...
                this.pos = beginPos;
                h.evalSet = new HashSet<Rule>(h.involvedSet);

                // Reparse the rule, trying to grow the seed.
                return new Continuation(3, m, lr, h);
            }
        ...
        case 3: {
            MemoEntry m = k.m; // memo[beginPos].get(rule);
            LR lr = k.lr; // ruleStack;
            Head h = k.h;
...
//            break;
    ...
    void removeErrors(final int fromPos, final int toPos) {
        ...
        for (final ListIterator<ParseError> i = errors.listIterator(); i.hasNext();) {
            ...
            if (fromPos <= e.pos && e.endPos <= toPos)
                i.remove();
        }
    }
...
//                    msgs.add("expected '" + expected + "' while parsing '" + whileParsing + "'");
                }
//                else
//                    msgs.add("expected '" + expected + "'");
            }
        }

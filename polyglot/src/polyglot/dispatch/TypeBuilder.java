...
public class TypeBuilder extends Visitor {
	...
	if (n.package_() != null) {
	    tb = tb.pushPackage(Types.get(n.package_().package_())).pushStatic(true);
	}
	...
	final LocalDef li = ts.localDef(n.position(), n.flags().flags(), n.typeNode().typeRef(), n.name().id());
	...
	if (n.body() != null) {
	    TypeBuilderContext tb2 = tb.pushAnonClass(n.position()).pushStatic(false);
    ...
    public Node visit(AmbTypeNode_c n, final TypeBuilderContext tb) throws SemanticException {
	n = (AmbTypeNode_c) acceptChildren(n, tb);
	if (n.typeRef() == null) {
	    final AmbTypeNode_c n1 = n;
	    final Ref<Type> r = Types.<Type> lazyRef(ts.unknownType(n.position()));
    ...
    public Node visit(AmbQualifierNode_c n, final TypeBuilderContext tb) throws SemanticException {
	n = (AmbQualifierNode_c) acceptChildren(n, tb);
	final AmbQualifierNode_c n1 = n;
	final Ref<Qualifier> r = Types.<Qualifier> lazyRef(ts.unknownQualifier(n.position()));
	...
	return n.typeRef(Types.<Type> ref(ts.arrayOf(n.position(), n.base().typeRef())));
    }

    public Node visit(CanonicalTypeNode_c n, final TypeBuilderContext tb) throws SemanticException {
...
//	System.out.println("tb = " + tb);
	...
	final Flags flags = ct.flags().isInterface() ? n.flags().flags().Public().Abstract() : n.flags().flags();
	...
	final Flags flags = ct.flags().isInterface() ? n.flags().flags().Public().Static().Final() : n.flags().flags();
	...
	if (defaultConstructorNeeded(n)) {
	    ...
	    TypeBuilderContext tb2 = tb.pushClass(n.classDef()).pushStatic(false);
	...
	for (final TypeNode tn : interfaces) {
		...
		public void handle(Type t) {
			...
			Globals.Compiler().errorQueue().enqueue(ErrorInfo.SEMANTIC_ERROR, e.getMessage(), tn.position());
	...
	if (superClass != null) {
		...
		public void handle(Type t) {
			...
			Globals.Compiler().errorQueue().enqueue(ErrorInfo.SEMANTIC_ERROR, e.getMessage(), superClass.position());
	    ...
	    if (Report.should_report(Report.types, 3))
		Report.report(3, "setting superclass of " + n.classDef() + " to " + s);
	    cd.superType(s);
	}
    ...
    private Ref<Type> cycleCheckingRef(final ClassDef cd, final TypeNode superClass) {
	Ref<Type> s = Types.<Type> lazyRef(ts.unknownType(superClass.position()));
	Callable<Type> resolver = cycleCheckingResolver(cd, superClass);
	s.setResolver(resolver, jobClock());
	return s;
    }

    private Callable<Type> cycleCheckingResolver(final ClassDef cd, final TypeNode superClass) {
	final Ref<? extends Type> t = superClass.typeRef();
	Callable<Type> resolver = new Callable<Type>() {
	    public Type call() {
		Type s = t.get();
		try {
		    System.out.println("checking cycle from " + s + " to " + cd);
	...
	LocalDef li = ts.localDef(n.position(), n.flags().flags(), n.typeNode().typeRef(), n.name().id());

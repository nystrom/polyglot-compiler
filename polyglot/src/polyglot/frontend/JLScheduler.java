...
public class JLScheduler extends Scheduler {
	...
	public Goal ImportTableInitialized(Job job) {
		...
		Goal g = new VisitorGoal("ImportTableInitialized", job,
				new InitImportsVisitor(job, ts, nf));
	...
	public Goal TypesInitialized(Job job) {
		// For this one, the goal is stored in the job. This is called a lot
		// from the system resolver and interning is expensive.
	...
	public Goal TypesInitializedForCommandLine() {
		...
		return new BarrierGoal("TypesInitializedForCommandLine",
				commandLineJobs()) {
	...
	public Goal TypeChecked(final Job job) {
		...
		return new VisitorGoal("TypeChecked", job, new NodeVisitor() {
			...
			// supertypes BEFORE calling isSubtype
	...
	public Goal ExceptionsChecked(final Job job) {
		...
		return new VisitorGoal("ExceptionsChecked", job, new NodeVisitor() {
			@Override
			public Node override(Node n) {
				Node m = n.accept(new ExceptionChecker(job, ts, nf),
						new ExceptionCheckerContext(job, ts, nf));
				m.accept(new ErrorReporter());
				return m;
			}
		}).intern(this);
	...
	public Goal ReachChecked(final Job job) {
		final TypeSystem ts = job.extensionInfo().typeSystem();
		final NodeFactory nf = job.extensionInfo().nodeFactory();
		final Map<Node, Ref<Boolean>> completes = (Map<Node, Ref<Boolean>>) job
				.get("completes");
		...
		return new VisitorGoal("ReachChecked", job, new NodeVisitor() {
			...
			public Node override(Node n) {
				...
				Node m2 = m.accept(new FwdReferenceChecker(job, ts, nf),
						(Object) null);
	...
	public Goal CodeGenerated(Job job) {
		TypeSystem ts = extInfo.typeSystem();
		NodeFactory nf = extInfo.nodeFactory();
		return new OutputGoal(job, new Translator(job, ts, nf,
				extInfo.targetFactory()));
	}

	// TODO: symbol cleanup
	// Don't have direct pointers from FieldInstance to FieldDef (and all other
	// Defs)
	// Instead, indirect through symbol table.
	// Don't copy data into FieldInstance. FI is _always_ lazily constructed
	// from an FD.
	// FI is subst(sigma, FD).
	// Then, can only modify defs.

	// Should be able to do InnerClassRemover in just one pass over the AST with
	// the inner class.
	// Should not have to update users of the class.

	// TODO: implement InnerClassRemover to be compatible with javac generated
	// code.
	...
	public Goal BytecodeCached(final Job job) {
		...
		return new VisitorGoal("BytecodeTranslator", job, new NodeVisitor() {
			...
			public Node override(Node n) {
				if (n instanceof SourceFile) {
					new BytecodeTranslator(job, ts, nf, bc)
							.visit((SourceFile) job.ast());
					return n;
				}
				return null;
			}

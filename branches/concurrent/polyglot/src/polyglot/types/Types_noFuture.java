...
public class Types_noFuture {
	...
	public static <T> Ref<T> ref(T v) {
		if (v == null)
			return null;
		Ref_noFuture_c<T> r = new Ref_noFuture_c<T>(v);
		return r;
	}

	public static <T> Ref<T> lazyRef() {
		// future and clock not set
		Ref_noFuture_c<T> r = new Ref_noFuture_c<T>();
		return r;
	}
	...
	public static <T> Ref<T> lazyRef(T defaultValue,
		...
		Ref_noFuture_c<T> r = new Ref_noFuture_c<T>(resolver, clock);
		r.updateSoftly(defaultValue);
		return r;
	}

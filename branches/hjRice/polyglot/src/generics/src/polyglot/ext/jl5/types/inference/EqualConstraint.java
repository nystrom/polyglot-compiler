...
public class EqualConstraint extends Constraint {
	...
	public List<Constraint> simplify() {
		...
		if (formal.isArray()) {
			...
			// CHECK anyway to get a context here ?
			if (formal_pt.baseType().typeEquals(actual_pt.baseType(), null)) {
				...
				for (int i = 0; i < n; i++) {
					...
					if (!(formal_targ instanceof Wildcard)
							&& !(actual_targ instanceof Wildcard)) {
						r.add(new EqualConstraint(actual_targ, formal_targ,
								solver));
						...
						r.add(new EqualConstraint(formal_targ_wc.bound(),
								actual_targ_wc.bound(), solver));
					}
				}
			}
		}

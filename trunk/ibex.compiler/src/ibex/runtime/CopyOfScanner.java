 ...
 */
public class CopyOfScanner implements IMatchContext {
    private static class Again3 implements IMatchResult {
        public Again3(int beginPos) {
        }
    }
    private static class Again2 implements IMatchResult {
        public Again2(int beginPos) {
        }
    }
    ...
    public CopyOfScanner(byte[] input) {
        this.input = input;
    ...
    IMatchResult again2(final Rule rule, int beginPos, IMatchResult answer) {
        MemoEntry m = memo[beginPos].get(rule);
        m.answer = answer;
    ...
    IMatchResult again3(IMatchResult answer1, int beginPos, Rule rule) {
        MemoEntry m = memo[beginPos].get(rule);
        ...
        if (answer1 == FAIL || this.pos <= m.endPos)
        {
            heads[beginPos] = null;
            this.pos = m.endPos;
            return m.answer;
    ...
    class Again implements IMatchResult { }
    
    private IMatchResult growRule(final Rule rule, IMatchResult answer) {
        MemoEntry m = new MemoEntry(answer, this.pos);
...
//    while (true) {
//        answer = memoRule("A");
//        if (answer == AGAIN) answer = this.A(); answer = growRule("A", answer);
//        if (answer == AGAIN2(bp)) answer = this.A(); answer = grow2("A", bp, answer);
//        prev = answer;
//    }
    ...
    IMatchResult growLR(final Rule rule, final int position, final MemoEntry m, final Head head) {
        ...
        while (true) {
            ...
            // If the seed didn't grow, break; restore the position to the point after the
            // last successful parse and return the last answer.
            if (answer == FAIL || this.pos <= m.endPos) break;

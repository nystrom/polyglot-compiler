...
public class ASTFragmenter extends ContextVisitor
{
    ...
    protected Node leaveCall(Node parent, Node old, Node n, NodeVisitor v) throws SemanticException {
        ...
        if (n instanceof ClassDecl) {
            ...
            fragmentMap.put(cd.classDef(), new ASTFragment(parent, cd, context()));
        }
        else if (n instanceof FieldDecl) {
            FieldDecl fd = (FieldDecl) n;
            ASTFragment fragment = new ASTFragment(parent, fd, context());
            ...
            fragmentMap.put(md.methodDef(), new ASTFragment(parent, md, context()));
        }
        else if (n instanceof ConstructorDecl) {
            ConstructorDecl cd = (ConstructorDecl) n;
            fragmentMap.put(cd.constructorDef(), new ASTFragment(parent, cd, context()));
        }
        else if (n instanceof LocalDecl) {
            LocalDecl ld = (LocalDecl) n;
            fragmentMap.put(ld.localDef(), new ASTFragment(parent, ld, context()));
        }
        else if (n instanceof Formal) {
            Formal f = (Formal) n;
            fragmentMap.put(f.localDef(), new ASTFragment(parent, f, context()));
        }
        else if (n instanceof Initializer) {
            Initializer i = (Initializer) n;
            fragmentMap.put(i.initializerDef(), new ASTFragment(parent, i, context()));
        }
        else if (n instanceof New) {
            New neu = (New) n;
            if (neu.anonType() != null) {
                fragmentMap.put(neu.anonType(), new ASTFragment(parent, neu, context()));
            }
        }

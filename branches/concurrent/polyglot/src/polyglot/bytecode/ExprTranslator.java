...
public class ExprTranslator extends AbstractExpTranslator {
	...
	if (n.left() instanceof Field) {
		...
		if (n.operator() != Assign.ASSIGN) {
		    visitChild(f.target());
		    il.DUP(n.position());
		    il.GETFIELD(typeof(f.fieldInstance().container()), f.fieldInstance().name().toString(), typeof(f.fieldInstance().type()), n.position());    
		    ...
		    coerce(il, typeof(n), typeof(f.fieldInstance().type()), n.position());
		    il.PUTFIELD(typeof(f.fieldInstance().container()), f.fieldInstance().name().toString(), typeof(f.fieldInstance().type()), n.position());    
		}
		else {
		    visitChild(f.target());
			...
			il.DUP_X1(n.position());  // S v e v  21 -> 121
		    coerce(il, typeof(n.right()), typeof(f.fieldInstance().type()), n.position());
		    il.PUTFIELD(typeof(f.fieldInstance().container()), f.fieldInstance().name().toString(), typeof(f.fieldInstance().type()), n.position());    
		}
	    }
	    else {
		if (n.operator() != Assign.ASSIGN) {
		    il.GETSTATIC(typeof(f.fieldInstance().container()), f.fieldInstance().name().toString(), typeof(f.fieldInstance().type()), n.position());    
		    il.uncheckedCoerce(typeof(n));
		    promote(n.right(), n.type());
		    ...
		    coerce(il, typeof(n), typeof(f.fieldInstance().type()), n.position());
		    il.PUTSTATIC(typeof(f.fieldInstance().container()), f.fieldInstance().name().toString(), typeof(f.fieldInstance().type()), n.position());    
		}
		else {
	...
	else if (n.left() instanceof Local) {
	    Local l = (Local) n.left();
	    if (n.operator() != Assign.ASSIGN) {
		visitChild(l); // S v
		visitChild(n.right()); // S v e
		binaryOp(n.operator(), typeof(n), n.position()); // S v+e
		il.dup(n.position());  // S v+e v+e
		coerce(il, typeof(n), typeof(l), n.position());
		store(l.localInstance(), n.position());  // S v+e
	    }
	    else {
		visitChild(n.right());   // S e
		il.dup(n.position());    // S e e      
		coerce(il, typeof(n.right()), typeof(l), n.position());
		store(l.localInstance(), n.position()); // S e
	    }
	}
	else if (n.left() instanceof ArrayAccess) {
	    ArrayAccess a = (ArrayAccess) n.left();
	    if (n.operator() != Assign.ASSIGN) {
		visitChild(a.array()); // S a
		visitChild(a.index()); // S a i
		il.DUP2(n.position());
	    ...
	    throw new InternalCompilerError("unexpected assign target");
	}
    }

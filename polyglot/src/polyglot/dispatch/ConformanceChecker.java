...
public class ConformanceChecker extends Visitor {
	...
	public Node visit(ClassDecl_c n) throws SemanticException {
		...
		if (type.superClass() != null) {
			if (!type.superClass().isClass()
					|| type.superClass().toClass().flags().isInterface()) {
				throw new SemanticException("Cannot extend non-class \""
						+ type.superClass() + "\".", n.position());
			}
			...
			if (type.typeEquals(ts.Object(), n.context())) {
				throw new SemanticException("Class \"" + n.classDef()
						+ "\" cannot have a superclass.", n.superClass()
						.position());
			}
		}
	...
	private void duplicateMemberClassCheck(ClassBody_c n)
			throws SemanticException {
		ClassDef type = n.context().currentClassDef();
		ArrayList<Ref<? extends Type>> l = new ArrayList<Ref<? extends Type>>(
				type.memberClasses());
			...
			for (int j = i + 1; j < l.size(); j++) {
				...
				if (mi instanceof Named && mj instanceof Named) {
					if (((Named) mi).name().equals(((Named) mj).name())) {
						throw new SemanticException("Duplicate member type \""
								+ mj + "\".", mj.position());
					}
				}
			}
	...
	private void duplicateConstructorCheck(ClassBody_c n)
			throws SemanticException {
		ClassDef type = n.context().currentClassDef();
		ArrayList<ConstructorDef> l = new ArrayList<ConstructorDef>(
				type.constructors());
	...
	public Node visit(ConstructorDecl_c n) throws SemanticException {
		...
		if (n.body() != null && flags.isNative()) {
			throw new SemanticException(
					"A native constructor cannot have a body.", n.position());
		}
		...
		if (fcontainer.isClass()) {
			...
			if (container.flags().isInterface()) {
				if (flags.isProtected() || flags.isPrivate()) {
					throw new SemanticException(
							"Interface members must be public.", n.position());
				}
			}

			// check that inner classes do not declare static fields, unless
			// they
			// are compile-time constants
			if (flags.isStatic() && container.isInnerClass()) {
				// it's a static field in an inner class.
				if (!flags.isFinal() || n.init() == null
						|| !n.init().isConstant()) {
					throw new SemanticException("Inner classes cannot declare "
	...
	private void checkMethodFlags(MethodDecl_c n, Flags flags)
			throws SemanticException {
		if (n.context().currentClass().flags().isInterface()) {
			if (flags.isProtected() || flags.isPrivate()) {
				throw new SemanticException(
						"Interface methods must be public.", n.position());
			}

			if (flags.isStatic()) {
				throw new SemanticException(
						"Interface methods cannot be static.", n.position());
			}
		}
	...
	private void overrideMethodCheck(MethodDecl_c n) throws SemanticException {
		...
		for (Iterator<MethodInstance> j = mi.implemented(n.context())
				.iterator(); j.hasNext();) {

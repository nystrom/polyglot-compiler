...
public class DispatchedTypeChecker {
    ...
    public DispatchedTypeChecker(Job job, TypeSystem ts, NodeFactory nf, Map<Node, Node> memo) {
	this.job = job;
	this.ts = ts;
	this.nf = nf;
	this.memo = memo;
	...
	assert n.context() != null : "null context for " + n + ": " + n.getClass().getName();

	try {
	    return n.accept(new TC(job, ts, nf), n.context());
	}
	catch (PassthruError e) {
	    if (e.getCause() instanceof SemanticException) {
		SemanticException x = (SemanticException) e.getCause();
		job.compiler().errorQueue().enqueue(ErrorInfo.SEMANTIC_ERROR, x.getMessage(), x.position() != null ? x.position() : n.position());
		return n;
	    }
	    else {
	...
	private TypeChecker tc(Context c) {
	    Map<Node, Node> memo = job.nodeMemo();
	    TypeChecker tc = new TypeChecker(job, ts, nf, memo);
	    tc = (TypeChecker) tc.context(c);
	    return tc;
	}

	public TC(Job job, TypeSystem ts, NodeFactory nf) {
	    this.job = job;
	    this.ts = ts;
	    this.nf = nf;
	}
	...
	private Assign_c typeCheckLeft(Assign_c n, Context context) throws SemanticException {
		...
		ContextVisitor tc1 = tc(context);
		Field left = (Field) a.left(tc1.nodeFactory());
	...
	public Node tcNew(New_c old, New_c n, Context context) throws SemanticException {
	    ...
	    TypeChecker tc = tc(context);
	    typeCheckFlags(n, tc);
	    typeCheckNested(n, tc);
	...
	private void typeCheckFlags(New_c n, TypeChecker tc) throws SemanticException {
	    ClassType ct = n.objectType().type().toClass();
	...
	private void typeCheckNested(New_c n, TypeChecker tc) throws SemanticException {
	    if (n.qualifier() != null) {
	...
	private New_c typeCheckerHeader(New_c n, Context context) throws SemanticException {
	    n = typeCheckObjectType(tc(context), n);
	...
	private New_c typeCheckObjectType(TypeChecker childtc, New_c n) throws SemanticException {
		...
		if (tn.type().isClass()) {
			...
			New k = findQualifier(childtc, n, ct);
	...
	private New findQualifier(TypeChecker childtc, New_c n, ClassType ct) throws SemanticException {
	    ...
	    if (outer.typeEquals(c.currentClass(), childtc.context())) {

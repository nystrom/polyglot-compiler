...
public class ReachSetup {
    ...
    public ReachSetup(TypeSystem ts, Map<Node, Ref<Collection<Name>>> breaks, Map<Node, Ref<Collection<Name>>> continues,
	    Map<Node, Ref<Collection<Type>>> exceptions, Map<Node, Ref<Boolean>> completes) {
	this.ts = ts;
	this.breaks = breaks;
	...
	completesRef(n).setResolver(new Callable<Boolean>() {
		...
		SwitchElement lastStmt = n.elements().get(n.elements().size() - 1);
	...
	completesRef(n).setResolver(new Callable<Boolean>() {
	    public Boolean call() {
		return (r.get() && !isEqual(n.cond(), true)) || b.get().contains(null);
	    }
	}, jobClock());
    }
	...
	final Name label = parent instanceof Labeled ? ((Labeled) parent).labelNode().id() : null;
	...
	completesRef(n).setResolver(new Callable<Boolean>() {
		...
		if (c.get().contains(null) && !isEqual(n.cond(), true))
		    return true;
		if (label != null && c.get().contains(label) && !isEqual(n.cond(), true))
		    return true;
	...
	completesRef(n).setResolver(new Callable<Boolean>() {
	    public Boolean call() {
		if (n.finallyBlock() != null && !completesRef(n.finallyBlock()).get())
		    return false;
	...
	if (n.alternative() == null) {
	    ...
	    completesRef(n).setResolver(new Callable<Boolean>() {
		public Boolean call() {
		    return completesRef(n.consequent()).get() || completesRef(n.alternative()).get();
		}
	    }, jobClock());
	}
    }
